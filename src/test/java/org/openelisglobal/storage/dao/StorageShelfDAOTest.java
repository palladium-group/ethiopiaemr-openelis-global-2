...
public class StorageShelfDAOTest extends BaseWebContextSensitiveTest {
    ...
    private void cleanTestData() {
        try {
            ...
            jdbcTemplate
                    .execute("DELETE FROM storage_shelf WHERE code IN ('DAO-SH-A', 'DAO-SH-B') OR id IN (9102, 9103)");
    ...
    public void testFindByLabel_WithValidLabel_ReturnsShelf() {
        ...
        // properties
        StorageShelf result = storageShelfDAO.findByLabel("DAO-Test Shelf A");
    ...
    public void testFindByParentDeviceId_WithValidDevice_ReturnsShelves() {
        ...
        assertTrue("Should contain shelf 1", results.stream().anyMatch(s -> s.getLabel().equals("DAO-Test Shelf A")));
        assertTrue("Should contain shelf 2", results.stream().anyMatch(s -> s.getLabel().equals("DAO-Test Shelf B")));
    }
    ...
    public void testFindByLabelAndParentDevice_WithValidParams_ReturnsShelf() {
        // Act: Execute HQL query
        StorageShelf result = storageShelfDAO.findByLabelAndParentDevice("DAO-Test Shelf A", testDevice);
