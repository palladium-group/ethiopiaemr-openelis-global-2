...
public class PatientDashBoardProviderTest extends BaseWebContextSensitiveTest {
    ...
    @Test
    public void getPendingReceptionQueue_withRole_groupsByAccessionAndIncludesPatientFields() throws Exception {
        performQueueGet("/rest/home-dashboard/PENDING_RECEPTION", receptionSession).andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.queue.totalItems").value(3))
                ...
                .andExpect(jsonPath("$.queue.items[2].testCount").value(2));
    }
    ...
    public void getPendingReceptionQueue_paginatesResults() throws Exception {
        mockMvc.perform(get("/rest/home-dashboard/PENDING_RECEPTION").param("page", "1").param("pageSize", "1")
                ...
                .andExpect(jsonPath("$.queue.pageSize").value(1)).andExpect(jsonPath("$.queue.totalItems").value(3))
                .andExpect(jsonPath("$.queue.totalPages").value(3))
