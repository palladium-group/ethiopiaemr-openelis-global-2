        ...
        async with streamable_http_client(url) as (read_stream, write_stream, _):
            ...
            async with ClientSession(
                ...
                assert required_tools.issubset(
                    tool_names
                ), f"Expected {required_tools}, got {tool_names}"
                ...
                assert not old_tools.intersection(
                    tool_names
                ), f"Old tools should not be present: {old_tools.intersection(tool_names)}"

                # Test get_query_context
                result = await session.call_tool(
                    "get_query_context", {"user_query": "find tests"}
                )
                ...
                # Test validate_sql
                result = await session.call_tool(
                    "validate_sql", {"sql": "SELECT * FROM test LIMIT 10"}
                )
                assert result.content, "Expected content in tool response."
    finally:
        process.terminate()
