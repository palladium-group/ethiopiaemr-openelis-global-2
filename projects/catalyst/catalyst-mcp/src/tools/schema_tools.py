...
def validate_sql(sql: str) -> dict[str, Any]:
    ...
    disallowed_tables = [
        t for t in referenced_tables if t.lower() not in allowed_tables
    ]

    if disallowed_tables:
        errors.append(
            f"Query references non-allowed tables: {', '.join(disallowed_tables)}"
        )
